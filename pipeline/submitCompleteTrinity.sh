...
case "$1" in
	...
	out=/proj/$proj/nobackup/somaticEmbryogenesis/trinity/all
	;;
    *) usage;;
esac
...
fwd=
for f in `find $in -type f -name "*_sortmerna_trimmomatic_1.fq.gz"`; do
    if [ -z $fwd ]; then
	fwd=$f
    else
	fwd=$fwd","$f
...
rev=
for f in `find $in -type f -name "*_sortmerna_trimmomatic_2.fq.gz"`; do
    if [ -z $rev ]; then
	rev=$f
    else
	rev=$rev","$f
...
if [ $runLoc == "umea" ]; then
    ...
    sbatch -t 4-00:00:00 -A $proj --mail-user $mail -C "usage_mail" -e $out/trinity.err -o $out/trinity.out -J trinity-$proj $UPSCb/pipeline/runTrinity.sh -m 100G -n $out $fwd $rev
fi
