...
out=/proj/$proj/somaticEmbryogenesis/HTSeq
...
do 
    fnam=`basename ${f//.bam/}`
    sbatch -A $proj --mail-user $mail -C "usage_mail" -e $out/$fnam.err -o $out/$fnam.out -J htseq-$fnam $UPSCb/pipeline/runHTSeq.sh -s $out $tmp $f $gff3
done
