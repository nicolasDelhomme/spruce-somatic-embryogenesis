...
# change names of the stages, so they cannot be converted to integers by mistake (append S to all the numbers, e.g. 1 to S1)
samples$Stages <- paste0("S", samples$Stages)
...
# write again file with different names of the Stages (with appended S)
write.csv(csamples,file="~/Git/UPSCb/projects/spruce-somatic-embryogenesis/doc/technical_samples-3_S.csv")
...
write.csv(counts,file="analysis/kallisto/raw-unormalised-gene-expression_data-3.csv")
...
#' Save it
#save(dds.counts,file="~/Git/UPSCb/projects/spruce-somatic-embryogenesis/DESeqDataSet-3.rda")  
# write again file with changed names of the Stages (with appended S)
save(dds.counts,file="~/Git/UPSCb/projects/spruce-somatic-embryogenesis/DESeqDataSet-3_S.rda")
...
#' ### blind
vsd.counts <- varianceStabilizingTransformation(dds.counts, blind=TRUE)
...
#' Write it out
write.csv(vst.counts,file="~/Git/UPSCb/projects/spruce-somatic-embryogenesis/library-size-corrected_blind-variance-stabilised_gene-expression_data-3.csv")
